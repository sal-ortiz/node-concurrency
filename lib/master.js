...
const BaseWorker = require(Path.join(__dirname, 'worker.js'));
...
class Master {
  ...
  startWorker() {
    let worker = new this.constructor.Worker();
  ...
  static get Worker() {
    return this._workerObj || BaseWorker;
  }

  static set Worker(newObj) {
    this._workerObj = newObj;
  }
