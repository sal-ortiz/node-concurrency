...
var Async = function(){
  ...
  var onExitCleanup = function(exitCode){
    // NOTE: Can potentially be called multiple times.
    try {
      // delete our swap file.
  ...
  for (var cpuNum = 0; cpuNum < numCPUs; cpuNum += 1){
    ...
    var worker = new Worker({
      ...
      'onMessage': function(msg){
        ...
        var promiseId = options.promise;

        // trigger the resolution of our promise.
        promises[promiseId].resolve.call(this, message['response']);

        // cleanup indicated value, if necessary.
        if( options['unset'] ){
          shared.unset(options['unset']);
        }
