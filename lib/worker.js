...
class Worker {
  ...
  static start(onMessage, env) {
    ...
    // TODO: verify that __filename accounts for inheritence.
    Cluster.setupMaster({
      exec: __filename,
  ...
  static getProcessArgs() {
    let reqs = this._requires || {};

    let reqArgs = Object.keys(reqs).map((name) => {
      let path = reqs[name];

      return ['--require', [name, '=', path].join()];
    });

    //let workerArgs = this._worker ? ['--worker', this._worker] : [];

    return [].concat(reqArgs/*, workerArgs*/);
  }

  static setRequire(name, path) {
    this._requires[name] = path;
  }
