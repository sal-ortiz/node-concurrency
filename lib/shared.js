...
Shared = function(options){
  ...
  var priv = {
    swap: options['swap'] || path.join(__dirname, ('.__npc.swap')),
    //encoding: 'base64',
    encoding: 'ascii',
    ...
    commit: function(){
      var payload = priv.serialize(priv.payload);
      fs.writeFile(priv.swap, payload, priv.encoding,
        function(err){
          if (err)  throw errors.commitMessage(err);
        }
      );
      return payload;
    },
  ...
  return {
    ...
    ping: {
      worker_message: "hi boss!",
      master_message: "get to work!",
    },
